#' .. content for \description{} (no empty lines) ..
#'
#' .. content for \details{} ..
#'
#' @title Monte-Carlo cross-validation simulation
#'
#' @param intermacs_fake the intermacs_fake target
#'
#' @param force_run If `TRUE`, then the simulation is run. Please note
#'   that each iteration of the simulation will take several hours on a
#'   standard computer. If `FALSE`, then the 'sim_results.csv' file
#'   will be read in and used as the simulation results, which takes
#'   basically no time at all.
#'
#' @param n_iter The number of iterations to run.
#'
#' @param test_index the test_index target, which contains information
#'   # on which rows go into the testing set for each iteration.
#'
run_sim <- function(intermacs_fake,
                    test_index,
                    n_iter = 10,
                    force_run = FALSE,
                    overwrite_sim_results_data = TRUE) {

  # require the number of iterations to match number of train/test splits.
  if(length(test_index) == n_iter){
    stop("the number of iterations (n_iter) must equal the ",
         "number of training/testing splits. Please adjust ",
         "one of these targets (i.e., n_iter or test_index) ",
         "and re-make the project.", call. = FALSE)
  }

  if(force_run | !file.exists('data/sim_results.csv')){
    .run_sim(
      intermacs_fake = intermacs_fake,
      test_index = test_index,
      n_iter = n_iter,
      overwrite_sim_results_data = overwrite_sim_results_data
    )
  } else {
    read_csv('data/sim_results.csv')
  }

}


.run_sim <- function(n_iter,
                     intermacs_fake,
                     test_index,
                     overwrite_sim_results_data){

  # Simulation parameters ---------------------------------------------------

  sim_params <- expand.grid(
    iteration = seq(n_iter),
    md_strat = c(
      'mia',
      'ranger_si',
      'meanmode_si',
      'ranger_mi',
      'pmm',
      'bayesreg',
      'hotdeck_si',
      'hotdeck_mi',
      'nbrs_mi',
      'nbrs_si'
    ),
    outcome = c('dead', 'txpl'),
    additional_missing_pct = c(0, 15, 30),
    stringsAsFactors = FALSE
  ) %>%
    as_tibble() %>%
    mutate(result = list(NULL))


  # Simulation loop ---------------------------------------------------------

  # In the original analysis, a parallel computing cluster
  # allowed this script to be executed a large number of times
  # across many cpu's. For convenience, we do not apply
  # parallel computation in this demonstration and instead
  # use the conventional for loop.

  # number of imputations to generate when using multiple imputation
  n_impute_mi = 5
  # horizon for computing predicted risks, i.e., 6 month mortality prediction
  times = 6
  # administrative censoring, i.e., events after 8 months don't count
  # a slight buffer is placed between the prediction horizon and admin
  # censoring to stabilize predicted probabilities by xgboost,
  # which uses events post prediction horizon to estimate the
  # hazard function
  maxtime = 8

  for(i in seq(nrow(sim_params))){

    # load simulation parameters
    # from ith row of sim_params
    # into the global environment:
    list2env(x = as.list(sim_params[i, ]), envir = .GlobalEnv)
    # this creates the iteration, md_strat, outcome,
    # and additional_missing_pct values for the current run

    msg <- glue("Beggining run number {i} \\
              \n--> missing data strategy: {md_strat} \\
              \n--> iteration: {iteration} \\
              \n--> outcome: {outcome} \\
              \n--> additional missing percent: {additional_missing_pct}")

    message(msg)

    # set random seed for reproducing the same additional
    # missing values for separate runs of the simulation
    # where the iteration is equal (i.e., different md_strat
    # but the same iteration)
    set.seed(iteration)

    # intermacs data for the current iteration
    im_iter <- intermacs_fake %>%
      rename(time = months_post_implant,
             status = paste0('pt_outcome_', outcome)) %>%
      select(-starts_with('pt_outcome_')) %>%
      # truncate event times
      mutate(
        status = if_else(time > maxtime & status == 1, 0, status),
        time = pmin(time, maxtime)
      )

    # testing index for the current iteration
    test_index_iter <- test_index$test_index[[iteration]]

    # train/test split -----
    im_train_raw <- im_iter[-test_index_iter, ]
    im_test_raw  <- im_iter[ test_index_iter, ]

    # remove variables with % missing > a threshold
    too_many_missing <- im_train_raw %>%
      miss_var_summary() %>%
      filter(pct_miss > 60 + additional_missing_pct) %>%
      pull(variable)

    im_train_raw[, too_many_missing] <- NULL
    im_test_raw[, too_many_missing] <- NULL

    prop_miss_train <- prop_miss(im_train_raw)
    prop_miss_test <- prop_miss(im_test_raw)

    features <- setdiff(names(im_train_raw), c('time', 'status'))

    message("% missing in training data: ", 100 * round(prop_miss_train,4))

    # amputation (if additional_missing_pct > 0) ----

    amputed_values <- vector(mode = 'list', length = length(features)) %>%
      set_names(features)

    # to induce MAR
    age_cats <- cut(im_train_raw$demo_age,
                    breaks = c(0, 40, 65, Inf),
                    include.lowest = TRUE)

    age_probs <- rep(0, length(age_cats))
    age_probs[age_cats == '[0,40]'] <- 0.40
    age_probs[age_cats == '(40,65]'] <- 0.10
    age_probs[age_cats == '(65,Inf]'] <- 0.40

    if(additional_missing_pct > 0){

      for(j in features){

        trn_n_observed <- sum(!is.na(im_train_raw[[j]]))
        trn_pct_observed <- 100 * trn_n_observed / nrow(im_train_raw)

        trn_n_to_ampute <- (additional_missing_pct/100) * nrow(im_train_raw)
        trn_n_to_ampute <- round(trn_n_to_ampute)

        if(trn_n_observed < trn_n_to_ampute){

          # ampute nothing, and remove feature from amputed value
          amputed_values[[j]] <- NULL

        } else {

          # make sure there are enough observed values in training data
          observed_index <- which(!is.na(im_train_raw[[j]]))

          ampute_index <- sample(x = observed_index,
                                 size = trn_n_to_ampute,
                                 replace = FALSE,
                                 prob = age_probs[observed_index])

          amputed_values[[j]] <- list(
            index = ampute_index,
            values = im_train_raw[[j]][ampute_index]
          )

          im_train_raw[[j]][ampute_index] <- NA

        }

      }

    }

    prop_miss_train <- prop_miss(im_train_raw)

    message(
      glue(
        "training sample: {nrow(im_train_raw)} ({sum(im_train_raw$status)})",
        " observations ({outcome} events) \n",
        "testing sample: {nrow(im_test_raw)} ({sum(im_test_raw$status)})",
        " observations ({outcome} events) \n",
        "{ncol(im_train_raw)-2} predictors"
      )
    )

    # create a pre-processing pipeline to be applied before imputation

    recipe_pre_impute <- recipe(
      formula = time + status ~ .,
      data = im_train_raw
    ) %>%
      # remove predictors with almost zero variance
      step_nzv(all_predictors()) %>%
      # collapse categories for predictors with near zero counts.
      step_other(all_nominal(), threshold = 0.025, other = 'Other_value')

    pipe_pre_impute <- prep(recipe_pre_impute)

    im_train_pre_impute <- juice(pipe_pre_impute)
    im_test_pre_impute <- bake(pipe_pre_impute, new_data = im_test_raw)

    # redefine features, since some features may have been dropped by step_nzv
    features <- setdiff(names(im_train_pre_impute), c('time', 'status'))

    # redefine amputed values to account for dropped features
    for(j in names(amputed_values)){
      if(!(j %in% features)) amputed_values[[j]] <- NULL
    }

    # missing value summary
    mv_summary <- miss_var_summary(im_train_pre_impute)

    message("Missing variables in training data")
    print(mv_summary, n = length(features)+2)

    # variables with missing values that miceranger will impute
    miceranger_var_names <- mv_summary %>%
      filter(n_miss > 0) %>%
      pull(variable)

    n_miss_vars <- length(miceranger_var_names)

    miceranger_vars <- vector(mode = 'list', length = n_miss_vars)

    for(j in seq_along(miceranger_vars)){

      names(miceranger_vars)[j] <- miceranger_var_names[j]
      # dont use the outcome to impute a predictor
      miceranger_vars[[j]] <- setdiff(features, miceranger_var_names[j])

    }

    # imputation ----

    pipe_meanmode_impute <- recipe_pre_impute %>%
      step_meanimpute(all_numeric()) %>%
      step_modeimpute(all_nominal()) %>%
      prep()

    # initialize all but fill only the imputations designated by md_strat
    mia <- meanmode_si <-
      pmm_mi <- pmm_si <-
      bayesreg_mi <- bayesreg_si <-
      hotdeck_mi <- hotdeck_si <-
      nbrs_mi <- nbrs_si <-
      ranger_mi <- ranger_si <- NULL

    if('mia' %in% md_strat){

      mia <- tibble(
        impute = 1,
        training = list(im_train_pre_impute),
        testing = list(im_test_pre_impute)
      )

    }
    if('meanmode_si' %in% md_strat){

      meanmode_si <- tibble(
        impute = 1,
        training = list(juice(pipe_meanmode_impute)),
        testing = list(bake(pipe_meanmode_impute, new_data = im_test_raw))
      )

    }
    if('pmm' %in% md_strat) {

      qp = im_train_pre_impute %>%
        select(-time, -status) %>%
        quickpred(mincor = 0.10, minpuc = 0.05)

      pmm_trn <- im_train_pre_impute %>%
        select(-time, -status) %>%
        mice(pred = qp,
             m = n_impute_mi,
             maxit = 10,
             method = 'pmm')

      pmm_tst <- im_test_pre_impute %>%
        select(-time, -status) %>%
        mice(pred = qp,
             m = n_impute_mi,
             maxit = 10,
             method = "pmm")

      pmm_mi <- tibble(
        impute = 1:n_impute_mi,
        training = map(complete(pmm_trn, 'all'), as_tibble),
        testing = map(complete(pmm_tst, 'all'), as_tibble)
      ) %>%
        # bind the outcomes back into the imputed data
        mutate(
          training = map(
            training,
            ~bind_cols(time = im_train_pre_impute$time,
                       status = im_train_pre_impute$status,
                       .x)
          ),
          testing = map(
            testing,
            ~bind_cols(time = im_test_pre_impute$time,
                       status = im_test_pre_impute$status,
                       .x)
          )
        )

      pmm_si <- slice(pmm_mi, 1)

    }
    if('bayesreg' %in% md_strat){

      qp = im_train_pre_impute %>%
        select(-time, -status) %>%
        quickpred(mincor = 0.10, minpuc = 0.05)

      bayesreg_trn <- im_train_pre_impute %>%
        select(-time, -status) %>%
        mice(pred = qp,
             m = n_impute_mi,
             maxit = 10,
             defaultMethod = c("norm","logreg","polyreg","polr"))

      bayesreg_tst <- im_test_pre_impute %>%
        select(-time, -status) %>%
        mice(pred = qp,
             m = n_impute_mi,
             maxit = 10,
             defaultMethod = c("norm","logreg","polyreg","polr"))

      bayesreg_mi <- tibble(
        impute = 1:n_impute_mi,
        training = map(complete(bayesreg_trn, 'all'), as_tibble),
        testing = map(complete(bayesreg_tst, 'all'), as_tibble)
      ) %>%
        mutate(
          training = map(
            training,
            ~bind_cols(time = im_train_pre_impute$time,
                       status = im_train_pre_impute$status,
                       .x)
          ),
          testing = map(
            testing,
            ~bind_cols(time = im_test_pre_impute$time,
                       status = im_test_pre_impute$status,
                       .x)
          )
        )

      bayesreg_si <- slice(bayesreg_mi, 1)

    }
    if('hotdeck_si' %in% md_strat) {

      numeric_features <- im_train_pre_impute %>%
        select(all_of(features)) %>%
        select(where(is.numeric)) %>%
        names()

      hotdeck_trn <- hotdeck_safe(data = im_train_pre_impute,
                                  ord_var = numeric_features[1:5],
                                  recipe = pipe_meanmode_impute)

      hotdeck_tst <- hotdeck_safe(data = im_test_pre_impute,
                                  ord_var = numeric_features[1:5],
                                  recipe = pipe_meanmode_impute)

      hotdeck_si <- tibble(
        impute = 1,
        training = list(as_tibble(hotdeck_trn)),
        testing = list(as_tibble(hotdeck_tst))
      )

    }
    if('hotdeck_mi' %in% md_strat) {

      numeric_features <- im_train_pre_impute %>%
        select(all_of(features)) %>%
        select(where(is.numeric)) %>%
        names() %>%
        .[1:n_impute_mi]

      hotdeck_trn <- map(
        .x = numeric_features,
        .f = ~ hotdeck_safe(data = im_train_pre_impute,
                            ord_var = .x,
                            recipe = pipe_meanmode_impute)
      )

      hotdeck_tst <- map(
        .x = numeric_features,
        .f = ~ hotdeck_safe(data = im_test_pre_impute,
                            ord_var = .x,
                            recipe = pipe_meanmode_impute)
      )

      hotdeck_mi <- tibble(
        impute = 1:n_impute_mi,
        training = map(hotdeck_trn, as_tibble),
        testing = map(hotdeck_tst, as_tibble)
      )

    }
    if('nbrs_mi' %in% md_strat){

      nbrs_mi_trn <- impute_nbrs(
        data_ref = im_train_pre_impute,
        cols = c(everything(), -time, -status),
        k_neighbors = round(seq(from = 2, to = 20, length.out = n_impute_mi)),
        aggregate = TRUE,
        verbose = TRUE
      )

      nbrs_mi_tst <- impute_nbrs(
        data_ref = im_test_pre_impute,
        cols = c(everything(), -time, -status),
        k_neighbors = round(seq(from = 2, to = 20, length.out = n_impute_mi)),
        aggregate = TRUE,
        verbose = TRUE
      )

      nbrs_mi <- nbrs_mi_trn %>%
        as_tibble() %>%
        transmute(
          impute,
          training = map(
            .x = imputed_values,
            .f = ~ipa:::fill_na(im_train_pre_impute, vals = .x)
          ),
          testing = map(
            .x = nbrs_mi_tst$imputed_values,
            .f = ~ipa:::fill_na(im_test_pre_impute, vals = .x)
          )
        )

    }
    if('nbrs_si' %in% md_strat){

      nbrs_si_trn <- impute_nbrs(
        data_ref = im_train_pre_impute,
        cols = c(everything(), -time, -status),
        k_neighbors = 10,
        aggregate = TRUE,
        verbose = TRUE
      )

      nbrs_si_tst <- impute_nbrs(
        data_ref = im_test_pre_impute,
        cols = c(everything(), -time, -status),
        k_neighbors = 10,
        aggregate = TRUE,
        verbose = TRUE
      )

      nbrs_si <- nbrs_si_trn %>%
        as_tibble() %>%
        transmute(
          impute,
          training = map(
            .x = imputed_values,
            .f = ~ipa:::fill_na(im_train_pre_impute, vals = .x)
          ),
          testing = map(
            .x = nbrs_si_tst$imputed_values,
            .f = ~ipa:::fill_na(im_test_pre_impute, vals = .x)
          )
        )

    }
    if('ranger_si' %in% md_strat){

      ranger_si_train <- miceRanger(
        data = im_train_pre_impute,
        m = 1,
        maxiter = 5,
        vars = miceranger_vars,
        returnModels = FALSE,
        verbose = TRUE,
        num.trees = 100
      ) %>%
        completeData() %>%
        set_names(1:length(.)) %>%
        enframe(name = 'impute', value = 'training') %>%
        mutate(training = map(training, as_tibble))

      ranger_si_test <- miceRanger(
        data = im_test_pre_impute,
        m = 1,
        maxiter = 5,
        vars = miceranger_vars,
        returnModels = FALSE,
        verbose = TRUE,
        num.trees = 100
      ) %>%
        completeData() %>%
        set_names(1:length(.)) %>%
        enframe(name = 'impute', value = 'testing') %>%
        mutate(testing = map(testing, as_tibble))

      ranger_si = left_join(ranger_si_train, ranger_si_test)

      rm(ranger_si_train, ranger_si_test)

    }
    if('ranger_mi' %in% md_strat){

      ranger_mi_train <- miceRanger(
        data = im_train_pre_impute,
        m = n_impute_mi,
        vars = miceranger_vars,
        maxiter = 5,
        meanMatchCandidates = 10,
        returnModels = FALSE,
        verbose = TRUE,
        num.trees = round(100 / n_impute_mi)
      ) %>%
        completeData() %>%
        set_names(1:length(.)) %>%
        enframe(name = 'impute', value = 'training') %>%
        mutate(training = map(training, as_tibble))

      ranger_mi_test <- miceRanger(
        data = im_test_pre_impute,
        m = n_impute_mi,
        vars = miceranger_vars,
        maxiter = 5,
        meanMatchCandidates = 10,
        returnModels = FALSE,
        verbose = TRUE,
        num.trees = round(100 / n_impute_mi)
      ) %>%
        completeData() %>%
        set_names(1:length(.)) %>%
        enframe(name = 'impute', value = 'testing') %>%
        mutate(testing = map(testing, as_tibble))

      ranger_mi <- left_join(ranger_mi_train, ranger_mi_test)

      rm(ranger_mi_train, ranger_mi_test)

    }

    # bind the imputations ----

    imputes <- bind_rows(
      mia = mia,
      meanmode_si = meanmode_si,
      pmm_mi = pmm_mi,
      pmm_si = pmm_si,
      bayesreg_mi = bayesreg_mi,
      bayesreg_si = bayesreg_si,
      hotdeck_mi = hotdeck_mi,
      hotdeck_si = hotdeck_si,
      nbrs_mi = nbrs_mi,
      nbrs_si = nbrs_si,
      ranger_si = ranger_si,
      ranger_mi = ranger_mi,
      .id = 'md_strat'
    )

    # score imputations ----
    # default values used for imputation scores
    # if no additional missingness is added

    impute_scores <- tibble(
      md_strat = unique(imputes$md_strat),
      numeric = NaN,
      nominal = NaN
    )

    # score the imputes if there is additional missingness
    if(additional_missing_pct > 0){

      impute_scores <- imputes %>%
        mutate(scores = map(training, score_imputes, amputed_values)) %>%
        unnest_wider(col = scores) %>%
        group_by(md_strat) %>%
        summarize(across(c(nominal, numeric), mean))

    }


    # predicting risk for event -----

    recipe_post_impute <-
      recipe(time + status ~ ., data = imputes$training[[1]]) %>%
      step_novel(all_nominal()) %>%
      step_dummy(all_nominal())

    risk_predictions <- try(
      imputes %>%
        group_by(md_strat) %>%
        summarize(
          cph = list(
            predict_risk_cph(
              training = training,
              testing = testing,
              pipeline = recipe_post_impute,
              verbose = FALSE,
              times = times
            )
          ),
          xgb = list(
            predict_risk_xgb(
              training = training,
              testing = testing,
              pipeline = recipe_post_impute,
              verbose = TRUE,
              times = times
            )
          ),
          .groups = 'drop'
        ) %>%
        pivot_longer(
          cols = -c(md_strat),
          values_to = 'sprobs',
          names_to = 'model'
        )
    )

    output <- try(
      risk_predictions %>%
        evaluate_risk_predictions(
          times = times,
          testing_data = select(im_test_raw, time, status)
        )  %>%
        left_join(impute_scores, by = 'md_strat') %>%
        mutate(
          prop_miss_train = prop_miss_train,
          prop_miss_test = prop_miss_test,
          .before = model
        )
    )

    sim_params$result[[i]] <- output

  }


  sim_output <- sim_params %>%
    select(-md_strat) %>%
    unnest(result)

  if(overwrite_sim_results_data)
    write_csv(sim_output, file = glue('sim_output.rds'))

  sim_output


}
